//如果宏已经被定义，则跳过内部代码
#ifndef CUSTOM_LIT_PASS_INCLUDE	//#ifndef = "if not defined"
#define CUSTOM_LIT_PASS_INCLUDE	//include保护：定义了一个宏，用于避免重复包含

#include "../ShaderLibrary/Common.hlsl"		//核心文件
#include "../ShaderLibrary/Surface.hlsl"	//物体表面结构体文件
#include "../ShaderLibrary/Shadows.hlsl"		//灯光阴影文件
#include "../ShaderLibrary/Light.hlsl"		//灯光结构体文件
#include "../ShaderLibrary/BRDF.hlsl"		//BRDF结构体文件
#include "../ShaderLibrary/Lighting.hlsl"	//计算光照文件
...
struct Varyings //变化量（片元着色器输入结构体）
{
	...
	float3 normalWS : VAR_NORMAL; //自定义语义，为了满足语法，无特殊意义，相当于使用 TEXCOORDn 语义
...
{	
	...
	surface.normal = normalize(input.normalWS); //顶点着色器中的法线数据到片元着色器中的法线数据会进行线性插值，会让部分法线长度变化，所以要进行归一化法线
	surface.viewDirection = normalize(_WorldSpaceCameraPos - input.positionWS); //视角方向 = 相机位置 - 片元位置
	surface.depth = -TransformWorldToView(input.positionWS).z; //将像素的世界空间坐标转换到相机空间，相机默认看向-z，所以取反获取正值
	surface.color = base.rgb;
	surface.alpha = base.a;
	surface.metallic = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _Metallic);
	surface.smoothness = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _Smoothness);
	//交错梯度噪声函数（像素坐标，帧数）：生成一个函数用于在两个级联中选择一个进行采样
	surface.dither = InterleavedGradientNoise(input.positionCS.xy, 0);

	#if defined(_PREMULTIPLY_ALPHA)
		BRDF brdf = GetBRDF(surface, true); //预乘Alpha模式下，控制物体的透明度也会影响其反射颜色，传入true参数让GetBRDF函数在计算漫反射颜色时乘上透明度
	#else
